...
public class SelectMemberOnlineActivity extends AppCompatActivity {
...
//    private Map<String,Boolean> istemselect=new HashMap<>();
    ...
    private List<MyNodeBean> defaultitem;
...
//        Log.e("aaa","父节点4id：");
        ...
        userlist = (List<layoutUser>)getIntent().getSerializableExtra("selectitem");
        defaultitem = (List<MyNodeBean>)getIntent().getSerializableExtra("defaultitem");
        ...
        if(orgRoot != null){
            ...
            if(orgChildNode != null){
                ...
                for (int i=0;i<orgChildNode.size();i++){
                    ...
                    boolean b = UIUtils.removeContactVirtual(name2);
                    String oldId = orgNode.getOldId();
                    if(type == 1){

                        if(!b){
//                            selectData(orgNode,oldId);
...
//                    bottomorgChildNode.add(orgNode);
        ...
        mDataLV.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                ...
                String json = GsonUtil.getJson(orgChildNode);
//                Log.e("aaaa","列表数据："+json);
...
//                }
                for (int k=0;k<orgChildNode.size();k++){
                    ...
                    String number = orgchildNodeitem.getData().getNumber();
                    String serialNumber = orgchildNodeitem.getSerialNumber();
                    if(type == 1){
//                        selectData(orgchildNodeitem,oldId+"");
...
//        tv_number.setText("已选成员:"+totalnum+">");
        if(userlist != null && userlist.size()>0){
            tv_number.setText("已选成员:"+userlist.size()+">");
            if(defaultitem != null){
                for (int i=0;i<defaultitem.size();i++){
                    MyNodeBean peopleBean = defaultitem.get(i);
        ...
        ll_lj.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if(TextUtils.equals(selectnum,"1")){
                    if(maps.size()>1){
                        Toast.makeText(SelectMemberOnlineActivity.this, "只能选择一个人", Toast.LENGTH_SHORT).show();
                        return;
                    }
...
//                YealinkApi.instance().meetNow(SelectMemberJoinActivity.this,meetnowcontact);
        ...
        tv_number.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(SelectMemberOnlineActivity.this,DataListActivity.class);
...
//                ImageView checkBox = (ImageView)view.findViewById(R.id.cb_agree);
                ...
                Log.e("ccc",""+name);
                String serialNumber = data.getNumber();
                String nodeId = orgNode.getNodeId();
                nodeBean.setSerialNumber(serialNumber);
                nodeBean.setName(name);
                nodeBean.setNodeId(nodeId);
...
//                if(meetnowcontact.size()>0){
//                    meetnowcontact.clear();
//                }
//                if(maps.){
//
//                }
                layoutUser layoutUser = userlist.get(j);
                ...
                if(TextUtils.equals(uid,oldId)){
                    ...
                    Log.e("fff","默认选中："+serialNumber1);


//                    int size = meetnowcontact.size();
//                    if(size>0){
//                        for (int k=0;k<size;k++){
//                            Contact contact = meetnowcontact.get(k);
//                            String serialNumber2 = contact.getSerialNumber();
//                            if(TextUtils.equals(serialNumber2,serialNumber1)){
//
//                            }
...
//                    }
                    for (int i=0;i<deletenodeBeens.size();i++){
                        ...
                        if(!maps.containsKey(serialNumber)){
                            ...
                            for (int p=0;p<userlist.size();p++){
                                layoutUser contact = userlist.get(p);
                                String serialNumber1 = contact.getUid();
                                if(TextUtils.equals(serialNumber1,uid)){
    ...
    public void initTab(String name, final String nodeId){
        ...
        index++;
        View inflate1 = View.inflate(SelectMemberOnlineActivity.this, R.layout.item_top_link, null);
...
//                            bottomorgChildNode.add(orgchildNodeitem);
    ...
    private void selectData(OrgNode orgchildNodeitem,String serialNumber) {
        for (int m=0;m<peopleOnLineBeenbeans.size();m++){
            ...
            String phone = peopleOnLineBean.getId();
//            if(phone.length()>=32){
//                if(TextUtils.equals(phone.substring(0,32),serialNumber)){
//                    Log.e("qqq",phone+",相等，"+serialNumber);
//                }
//            }

//            Log.e("qqq",phone.substring(0,3)+",相等，");
            Log.e("vvv",phone+",比较，"+serialNumber);
    ...
    class MyLvAdapter extends BaseAdapter {
        ...
        public View getView(final int i, View convertView, ViewGroup viewGroup) {
            ...
            if (convertView == null) {
                convertView = LayoutInflater.from(SelectMemberOnlineActivity.this).inflate(R.layout.item_lv_childlj,null);
    ...
    class MyLvAdapter1 extends BaseAdapter {
        ...
        public View getView(final int i, View convertView, ViewGroup viewGroup) {
            ...
            if (convertView == null) {
                convertView = LayoutInflater.from(SelectMemberOnlineActivity.this).inflate(R.layout.item_lv_child1lj,null);
            ...
            OrgNodeData data = orgNode.getData();
            String name = data.getName();
            String json = GsonUtil.getJson(orgNode);
            Log.e("mmm",name+",号1码："+json);
            String serialNumber = data.getNumber();

//            String name = orgNode.getName();
//            String serialNumber = orgNode.getSerialNumber();
//            Log.e("mmm",name+",号1码："+serialNumber);
            final String nodeId = orgNode.getNodeId();
            final int size = orgNode.getData().getCount();
