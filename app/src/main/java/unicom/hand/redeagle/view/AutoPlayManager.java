package unicom.hand.redeagle.view;
		...
		if (broadcastEnable) {
			...
			if (mImageIndicatorView.getCurrentIndex() < mImageIndicatorView
				...
				if (mImageIndicatorView.getCurrentIndex() == mImageIndicatorView
					...
					// direction = LEFT;
					// 让图片滚到第一个
					mImageIndicatorView.getViewPager().setCurrentItem(0, true);
				} else {
					// 滚到下一张图片
					mImageIndicatorView.getViewPager().setCurrentItem(
							mImageIndicatorView.getCurrentIndex() + 1, true);
				}
			}

			broadcastHandler.sendEmptyMessageDelayed(0, this.intevalMils);
		}
	}
